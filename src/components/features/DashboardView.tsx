...
// Tab components (lazy via next/dynamic is handled inside each component)
...
// Map new tab categories to OBD2AnalysisResult keys for CategoryPanel
...
function ResetZoomButton() {
  const { isRangeActive, resetTimeRange } = useTimeRange();
  ...
  return (
    <Button
      ...
      className="text-subaru-red hover:text-subaru-red/80 border border-subaru-red/30 hover:border-subaru-red/60 transition-colors"
    >
      Reset Zoom
    </Button>
  );
...
export function DashboardView() {
  ...
  const [activeCategory, setActiveCategory] = useState<(typeof CATEGORY_ORDER)[number]>("overview");
  ...
  const hasChartData = timeSeries.length > 0 && derived && thresholds;
                    ...
                    {CATEGORY_ORDER.map((cat) => {
                      ...
                      return (
                        <TabsContent key={cat} value={cat}>
                          {/* Existing metrics summary for categories that have them */}
                          {resultKey && (
                            <CategoryPanel
                              category={cat}
