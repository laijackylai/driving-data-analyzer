...
export function CobbPowerTab({ timeSeries }: CobbPowerTabProps) {
  ...
  // Use accelPosition for COBB WOT detection
  const pulls = useMemo(() => detectWOTPulls(filteredTimeSeries, "accelPosition"), [filteredTimeSeries]);
  ...
  // ECU torque data for cross-reference chart
  const ecuTorqueData = useMemo(() => {
    const rpms: number[] = [];
    const torques: number[] = [];
    for (const d of filteredTimeSeries) {
      if (typeof d.reqTorqueNm === "number" && typeof d.engineRpm === "number") {
        rpms.push(d.engineRpm);
        torques.push(d.reqTorqueNm);
      }
    }
    return { rpms, torques };
  }, [filteredTimeSeries]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Chart 1: Dyno Chart — best pull */}
      {hasWeight && (
        <ChartWrapper title="Dyno Chart (Best Pull)" height={300} tooltipContent={<MetricTooltip content={METRIC_TOOLTIPS.dynoChart} />}>
      ...
      {/* Chart 2: Per-pull overlay */}
      {hasWeight && hasPulls && (
        <ChartWrapper title="Per-Pull HP Overlay" height={320} tooltipContent={<MetricTooltip content={METRIC_TOOLTIPS.dynoChart} />}>
      ...
      {/* Chart 3: ECU Torque vs Estimated Torque (COBB-exclusive) */}
      {hasWeight && hasPulls && (
        <ChartWrapper title="ECU Torque vs Estimated Torque" height={300}>
          {(() => {
            // Estimated engine torque from best pull (wheel torque / drivetrain ratio)
            const estimatedTraces: Plotly.Data[] = [];
            if (bestPullData && bestPullData.accelPower.length > 0) {
              const gear = bestPullData.pull.gear !== undefined ? Math.round(bestPullData.pull.gear) : 0;
              const ratio = WRX_GEAR_RATIOS[gear];
              if (ratio) {
                estimatedTraces.push({
                  x: bestPullData.accelPower.map((p) => p.rpm),
                  y: bestPullData.accelPower.map((p) => p.wheelTorqueNm / (ratio * WRX_FINAL_DRIVE)),
                  type: "scatter" as const,
                  mode: "lines" as const,
                  name: "Estimated Engine Torque (Nm)",
                  line: { color: CHART_COLORS.primary, width: 2 },
                });
              }
            }
            if (ecuTorqueData.rpms.length === 0 && estimatedTraces.length === 0) {
              return <NoDataMessage message="No ECU torque or estimated torque data available" />;
            }
            return (
              <Plot
                data={[
                  ...(ecuTorqueData.rpms.length > 0
                    ? [
                        {
                          x: ecuTorqueData.rpms,
                          y: ecuTorqueData.torques,
                          type: "scatter" as const,
                          mode: "markers" as const,
                          name: "ECU Requested Torque (Nm)",
                ...
                layout={{
                  ...
                  height: 300,
                  xaxis: { ...BASE_LAYOUT.xaxis, title: { text: "RPM", font: { size: 11, color: CHART_COLORS.textMuted } } },
                  yaxis: { ...BASE_LAYOUT.yaxis, title: { text: "Torque (Nm)", font: { size: 11, color: CHART_COLORS.textMuted } } },
                  legend: { ...BASE_LAYOUT.legend, x: 0.02, y: 0.98 },
                  showlegend: true,
                }}
                config={BASE_CONFIG}
      ...
      {/* Chart 4: Power vs Gear overlay */}
      {hasWeight && hasPulls && (
        <ChartWrapper title="Power vs Gear" height={300}>
          {(() => {
      ...
      {/* Chart 5: Peak HP/Torque trend per pull */}
      {hasWeight && hasPulls && (
        <ChartWrapper title="Peak HP / Torque per Pull" height={280} tooltipContent={<MetricTooltip content={METRIC_TOOLTIPS.peakTrend} />}>
      ...
      {/* Chart 6: Power-to-Weight vs Speed */}
      {hasWeight && hasPulls && (
        <ChartWrapper title="Power-to-Weight Ratio" height={280} tooltipContent={<MetricTooltip content={METRIC_TOOLTIPS.powerToWeight} />}>
          {(() => {
            const speeds: number[] = [];
            const ratios: number[] = [];
            for (const d of pullPowerData) {
              for (let i = 0; i < d.accelPower.length; i++) {
                const pt = d.pull.points[Math.min(i, d.pull.points.length - 1)];
      ...
      {/* MAF-based power — always visible */}
      <ChartWrapper title="MAF-Based Engine Power vs RPM" height={280}>
